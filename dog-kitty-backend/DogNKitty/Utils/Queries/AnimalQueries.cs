...
namespace Utils.Queries
{
    ...
    {
        public static string GET_ANIMAL = "select  a.id,a.nome,status,peso,idade,sexo,porte,tipoanimal,f.id,imagem,f.animal_id,r.id,r.nome,r.animal_id from kittydoggy.animal a left join kittydoggy.raca r on r.animal_id = a.id left join kittydoggy.foto f on f.animal_id  = a.id WHERE 1=1";
